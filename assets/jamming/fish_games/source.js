let sketch = function (p) {
    ...
    function saveImage() {
        const timestamp =
            p.year() + p.nf(p.month(), 2) + p.nf(p.day(), 2) + "-" +
    ...
    p.setup = function () {
        ...
        // let shapeGraphic = p.createGraphics(400, 400);
        // let textureGraphic = p.createGraphics(400, 400);

        let shapeGraphic = p.createGraphics(128, 128);
        let textureGraphic = p.createGraphics(128, 128);

        let maskColor = p.color(0, 0, 100);
        let alignmentColor = p.color(255, 0, 100);

        generateFish(p, shapeGraphic, textureGraphic, maskColor, alignmentColor);

        p.image(shapeGraphic, 0, 0);
        p.image(textureGraphic, shapeGraphic.width, 0);
    };
    ...
    function generateFish(p, shapeGraphic, textureGraphic, maskColor, alignmentColor) {
